...
- Import snapshots only on a trusted device. The app validates them locally, strips unknown fields, stores the result in the current tab session, and removes it on sign-out, tab close, or after 15 minutes without activity.
...
The prototype protects against accidental repository disclosure, persistence of private snapshots across browser sessions, unattended open tabs through a 15-minute inactivity lock, unknown JSON fields, malformed arithmetic, and obvious unmasked bank labels. It does not protect against a compromised device, malicious browser extension, shared access during the active window, developer-tools access, or hostile scripts introduced through a future code change.
