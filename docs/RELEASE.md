...
3. Desktop and mobile smoke tests cover the zero-data gate, JSON import, a fully expanded import dialog within the dynamic viewport, stale-data state, PEAK-only navigation, invalid import, exit removal, inactivity warning, automatic lock, render-failure recovery, and failed-deletion guidance.
